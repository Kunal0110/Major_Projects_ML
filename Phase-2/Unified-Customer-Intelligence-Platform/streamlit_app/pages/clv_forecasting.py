...
from streamlit_app.utils.api_client import predict_clv
...
st.title("💰 CLV Forecasting Hub")
st.info(f"👤 Logged in as: {user['name']} | 📧 {user['email']}")

st.subheader("Predict Individual CLV")
...
if submit:
    ...
    result = predict_clv(payload)
    if "clv" in result:
        st.success(f"Predicted Customer Lifetime Value: **${result['clv']:.2f}**")
    else:
...
if file:
    ...
    for _, row in df.iterrows():
        pred = predict_clv(row.to_dict())
        predictions.append(pred.get("clv", 0))
